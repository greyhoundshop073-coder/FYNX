...
export function registerR6GIntegrationRoutes({ app }) {
  ...
  const listingContext = async (listingId) => {
  ...
  app.get('/api/r6g/listings/:id/context', auth, async (req, res) => {
    try {
      ...
      return res.json({ context: {
        listingId: String(row.id), sellerId: String(row.seller_id), sellerUsername: row.seller_username, sellerDisplayName: row.seller_display_name,
        businessId: row.business_id == null ? null : String(row.business_id), businessName: row.business_name || null,
        businessUsername: row.business_username || null, businessCategory: row.business_category || null,
        businessVerified: Boolean(row.business_verified), businessActive: row.business_active == null ? null : Boolean(row.business_active),
        active: Boolean(row.active), quantity: Number(row.quantity), storeName: row.store_name, title: row.title,
  ...
  app.patch('/api/r6g/listings/:id/business', auth, async (req, res) => {
    ...
    try {
      ...
      if (String(listing.seller_id) !== String(req.user.sub)) return res.status(403).json({ error: 'only the listing seller can change business ownership' });
      ...
      if (businessId != null) {
        const business = (await client.query('SELECT id FROM business_profiles WHERE id=$1 AND owner_id=$2 AND COALESCE(active,TRUE)=TRUE LIMIT 1', [businessId, req.user.sub])).rows[0];
        if (!business) return res.status(403).json({ error: 'business is not owned by this account' });
      }
    ...
    } catch (error) { console.error('r6g listing business link', error); return res.status(500).json({ error: 'business linkage failed' }); }
  ...
  app.post('/api/r6g/listings/:id/share', auth, async (req, res) => {
    ...
    try {
      ...
      const listingId = positiveId(req.params?.id);
      if (!listingId) return res.status(400).json({ error: 'invalid listing id' });
      const listing = (await client.query(`SELECT l.*,b.id AS linked_business_id,b.business_name,b.business_username FROM marketplace_listings l LEFT JOIN business_profiles b ON b.id=l.business_id WHERE l.id=$1 AND l.active=TRUE AND l.quantity>0 LIMIT 1`, [listingId])).rows[0];
      if (!listing) return res.status(404).json({ error: 'listing unavailable' });
      if (String(listing.seller_id) !== String(req.user.sub)) return res.status(403).json({ error: 'only the listing seller can share this product from its canonical record' });
    ...
    } catch (error) { console.error('r6g listing share', error); return res.status(500).json({ error: 'product share failed' }); }
    finally { client.release(); }
  });
  ...
  app.post('/api/r6g/messages/:id/context', auth, async (req, res) => {
    try {
      await ensureSchema();
      const messageId = positiveId(req.params?.id);
      const listingId = req.body?.listingId == null ? null : positiveId(req.body.listingId);
      if (!messageId || !listingId) return res.status(400).json({ error: 'valid message and listing ids are required' });
      const message = (await pool.query('SELECT id,sender_id,recipient_id FROM messages WHERE id=$1 LIMIT 1', [messageId])).rows[0];
      ...
      const listing = await listingContext(listingId);
      if (!listing) return res.status(404).json({ error: 'listing not found' });
      ...
      if (listing.business_id != null && String(listing.business_id) !== '' && String(req.body?.businessId || listing.business_id) !== String(listing.business_id)) return res.status(400).json({ error: 'business context does not match listing' });
      const businessId = listing.business_id == null ? null : String(listing.business_id);
      await pool.query('UPDATE messages SET listing_id=$1,business_id=$2 WHERE id=$3', [listingId, listing.business_id || null, messageId]);
      return res.json({ messageId: String(messageId), listingId: String(listingId), businessId });
  ...
  app.post('/api/r6g/groups/:groupId/marketplace-posts', auth, async (req, res) => {
    try {
      ...
      const listingId = positiveId(req.body?.listingId);
      const text = typeof req.body?.text === 'string' ? req.body.text.trim().slice(0, 4000) : '';
      if (!groupId || !listingId) return res.status(400).json({ error: 'group and listing are required' });
      const member = (await pool.query('SELECT role FROM fynx_group_members WHERE group_id=$1 AND user_id=$2 LIMIT 1', [groupId, req.user.sub])).rows[0];
      if (!member) return res.status(403).json({ error: 'group membership required' });
      const listing = await listingContext(listingId);
      if (!listing || !listing.active || Number(listing.quantity) <= 0) return res.status(404).json({ error: 'listing unavailable' });
      const blocked = await pool.query(`SELECT 1 FROM blocks WHERE (blocker_id=$1 AND blocked_id=$2) OR (blocker_id=$2 AND blocked_id=$1) LIMIT 1`, [req.user.sub, listing.seller_id]);
      if (blocked.rowCount && String(listing.seller_id) !== String(req.user.sub)) return res.status(403).json({ error: 'listing unavailable' });
      const post = (await pool.query('INSERT INTO fynx_group_posts(id,group_id,author_id,text,listing_id,business_id) VALUES(gen_random_uuid(),$1,$2,$3,$4,$5) RETURNING id,created_at', [groupId, req.user.sub, text, listingId, listing.business_id || null])).rows[0];
      ...
      console.error('r6g group marketplace post', error);
      return res.status(500).json({ error: 'group product share failed' });
    }
  });

  app.get('/api/r6g/health', auth, async (_req, res) => {
    try {
      await ensureSchema();
      const checks = await Promise.all([
        pool.query("SELECT 1 FROM information_schema.columns WHERE table_name='marketplace_listings' AND column_name='business_id'"),
        pool.query("SELECT 1 FROM information_schema.columns WHERE table_name='social_posts' AND column_name='listing_id'"),
        pool.query("SELECT 1 FROM information_schema.columns WHERE table_name='messages' AND column_name='listing_id'"),
        pool.query("SELECT 1 FROM information_schema.columns WHERE table_name='fynx_group_posts' AND column_name='listing_id'")
      ]);
      return res.json({ ok: checks.every(r => r.rowCount > 0), canonicalListingLinks: true, businessLinks: true, homeLinks: true, messageLinks: true, groupLinks: true });
    } catch (error) { return res.status(500).json({ ok: false, error: 'integration schema unavailable' }); }
  });
}
