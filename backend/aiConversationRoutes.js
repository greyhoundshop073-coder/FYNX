...
export function registerFynxAiConversationRoutes({ app }) {
  ...
  app.post("/api/assistant/conversations/:id/message", async (req,res)=>{
    ...
    try {
      ...
      const input=[
        ...history.map(item=>({role:item.role,content:[{type:"input_text",text:item.text}]})),
        {role:"user",content:[...(message?[{type:"input_text",text:message}]:[]),...images.map(image=>({type:"input_image",image_url:image.dataUrl}))]}
      ];
      ...
      const response=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{Authorization:`Bearer ${OPENAI_API_KEY}`,"Content-Type":"application/json"},body:JSON.stringify({
        model:OPENAI_MODEL,
        instructions:"You are FYNX AI inside the FYNX app. Use approved FYNX tools when appropriate. Be concise, helpful and honest. Treat database results from tools as authoritative only for the authenticated user. Never claim an action happened unless a tool completed it. Never expose secrets or private data.",
        input,store:false
      })});
