...
app.get("/health", async (_req, res) => {
  ...
  res.json({ ok: database !== "unavailable", service: "fynx-backend", database });
});
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    return res.json({ user: { id: user.id, username: user.username, created_at: user.created_at }, accessToken: issueToken(user) });
  } catch (error) {
...
app.get("/api/me", auth, async (req, res) => {
  try {
    const user = await pool.query("SELECT id, username, created_at FROM users WHERE id = $1", [req.user.sub]);
  ...
  try {
    ...
    const result = await pool.query(
      `SELECT m.id, m.sender_id, sender.username AS sender_username,
              m.recipient_id, recipient.username AS recipient_username,
              m.text, EXTRACT(EPOCH FROM m.created_at) * 1000 AS timestamp,
              m.edited, m.deleted, m.reply_to_id
         FROM messages m
        ...
        LIMIT 200`,
      [req.user.sub, other.id]
    );
    return res.json({ messages: result.rows });
  } catch (error) {
    console.error("messages", error);
    return res.status(500).json({ error: "message history failed" });
  }
});
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO messages (sender_id, recipient_id, text, reply_to_id)
       VALUES ($1, $2, $3, $4)
       RETURNING id, sender_id, recipient_id, text,
                 EXTRACT(EPOCH FROM created_at) * 1000 AS timestamp,
                 edited, deleted, reply_to_id`,
      [req.user.sub, recipient.id, text, Number.isInteger(replyToId) ? replyToId : null]
    );
...
app.post("/api/assistant", auth, async (req, res) => {
  try {
    ...
    const response = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({ model: OPENAI_MODEL, input: message })
    });
    const data = await response.json();
    if (!response.ok) {
      console.error("openai", response.status, data?.error?.message || "request failed");
    ...
    return res.status(503).json({ error: "AI service is not configured or unavailable" });
  }
});
