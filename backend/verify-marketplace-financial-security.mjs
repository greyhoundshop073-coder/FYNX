...
const checks = [
  ...
  ['payment initialization validates provider reference amount and currency', reputation.includes('providerReference !== reference') && reputation.includes('providerAmount !== amount') && reputation.includes('providerCurrency !== String(existing.currency).toUpperCase()')],
  ['payment verification binds provider reference to requested reference', reputation.includes('providerReference === reference')],
  ['payment verification binds provider buyer metadata', reputation.includes('metadataBuyerId === String(order.buyer_id)')],
  ['payment webhook binds buyer metadata to the order', webhook.includes('metadataBuyerId !== String(order.buyer_id)')],
