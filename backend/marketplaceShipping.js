...
function normalizeCoverageRow(value) {
  ...
  return {
    ...
    state: normalizeText(value.state, 100).toUpperCase(),
    city: normalizeText(value.city, 100).toUpperCase()
  };
}

export async function isMarketplaceDestinationCovered(client, listingId, address) {
  const rows = (await client.query(
    'SELECT country,state,city FROM marketplace_shipping_coverage WHERE listing_id=$1 ORDER BY id ASC',
    [listingId]
  )).rows;
  ...
  return rows.some((row) =>
    String(row.country).toUpperCase() === country &&
    (!row.state || String(row.state).toUpperCase() === state) &&
    (!row.city || String(row.city).toUpperCase() === city)
  );
}
  ...
  app.get('/api/marketplace/listings/:id/shipping', auth, async (req, res) => {
    ...
    try {
      ...
      const listing = (await client.query(
        `SELECT id,seller_id,shipping_method,shipping_note,delivery_fee,additional_item_shipping_fee,shipping_fee_cap,delivery_available,pickup_available
         FROM marketplace_listings WHERE id=$1 LIMIT 1`,
        [listingId]
      )).rows[0];
      ...
      return res.json({ shipping: {
        ...
        note: listing.shipping_note,
        baseFee: Number(listing.delivery_fee || 0),
        additionalItemFee: Number(listing.additional_item_shipping_fee || 0),
        feeCap: listing.shipping_fee_cap == null ? null : Number(listing.shipping_fee_cap),
        deliveryAvailable: Boolean(listing.delivery_available),
        pickupAvailable: Boolean(listing.pickup_available),
        coverage: coverage.map((row) => ({ country: row.country, state: row.state, city: row.city }))
      }});
    } catch (error) {
      console.error('[marketplace-shipping-get]', error);
      return res.status(500).json({ error: 'shipping settings could not be loaded' });
    } finally { client.release(); }
  });

  app.put('/api/marketplace/listings/:id/shipping', auth, async (req, res) => {
    const listingId = positiveInt(req.params?.id);
    if (!listingId) return res.status(400).json({ error: 'invalid listing id' });
    const client = await pool.connect();
    try {
      ...
      const listing = (await client.query(
        'SELECT id,seller_id,delivery_available,pickup_available FROM marketplace_listings WHERE id=$1 FOR UPDATE',
        [listingId]
      )).rows[0];
      ...
      if (baseFee == null || additionalItemFee == null || (req.body?.feeCap != null && req.body?.feeCap !== '' && feeCap == null)) {
        await client.query('ROLLBACK');
        return res.status(400).json({ error: 'shipping fees must be valid non-negative amounts' });
      }
      if (feeCap != null && feeCap < baseFee) {
        await client.query('ROLLBACK');
        return res.status(400).json({ error: 'shipping fee cap cannot be below the base delivery fee' });
      }
      if (!Boolean(listing.delivery_available) && (baseFee > 0 || additionalItemFee > 0 || feeCap != null)) {
        await client.query('ROLLBACK');
        return res.status(409).json({ error: 'delivery is disabled for this listing' });
      }
      ...
      if (coverage.some((row) => !row)) {
        await client.query('ROLLBACK');
        return res.status(400).json({ error: 'each coverage entry requires a country' });
      }
      const uniqueCoverage = [];
      const seen = new Set();
      for (const row of coverage) {
        const key = `${row.country}|${row.state}|${row.city}`;
        if (!seen.has(key)) { seen.add(key); uniqueCoverage.push(row); }
      }
      if (uniqueCoverage.length > 100) {
        await client.query('ROLLBACK');
        return res.status(400).json({ error: 'too many delivery coverage entries' });
      }

      await client.query(
        `UPDATE marketplace_listings SET shipping_method=$2,shipping_note=$3,delivery_fee=$4,additional_item_shipping_fee=$5,shipping_fee_cap=$6,updated_at=NOW() WHERE id=$1`,
        [listingId, shippingMethod, note, baseFee, additionalItemFee, feeCap]
      );
      await client.query('DELETE FROM marketplace_shipping_coverage WHERE listing_id=$1', [listingId]);
      for (const row of uniqueCoverage) {
        await client.query(
          'INSERT INTO marketplace_shipping_coverage(listing_id,country,state,city) VALUES($1,$2,$3,$4)',
          [listingId, row.country, row.state, row.city]
        );
      }
      ...
      console.error('[marketplace-shipping-update]', error);
      return res.status(500).json({ error: 'shipping settings could not be saved' });
    } finally { client.release(); }
  });
