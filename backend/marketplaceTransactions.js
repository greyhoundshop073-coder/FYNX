export function registerMarketplaceTransactionRoutes({ app, pool, auth }) {
  ...
  const publicOrder = (row) => ({
    ...
    quantity: Number(row.quantity), unitPrice: Number(row.unit_price), deliveryFee: Number(row.delivery_fee), totalAmount: Number(row.total_amount),
    currency: row.currency, product: row.product_snapshot, status: row.status, paymentReference: row.payment_reference || null,
    trackingReference: row.tracking_reference || null, shippedAt: row.shipped_at, deliveredAt: row.delivered_at,
    inspectionDeadline: row.inspection_deadline, completedAt: row.completed_at, cancelledAt: row.cancelled_at,
  ...
  app.post('/api/marketplace/orders', auth, async (req, res) => {
    ...
    try {
      ...
      const listingResult = await client.query(`SELECT l.*, u.username AS seller_username, u.display_name AS seller_display_name FROM marketplace_listings l JOIN users u ON u.id = l.seller_id WHERE l.id = $1 AND l.active = TRUE FOR UPDATE`, [listingId]);
      ...
      if (quantity > available) { await client.query('ROLLBACK'); return res.status(409).json({ error: 'requested quantity is not available' }); }
      if (clientOrderId) {
        const existing = await client.query('SELECT * FROM marketplace_orders WHERE id = $1 AND buyer_id = $2', [clientOrderId, req.user.sub]);
        if (existing.rows[0]) { await client.query('ROLLBACK'); return res.status(200).json({ order: publicOrder(existing.rows[0]), idempotent: true }); }
      }
      const orderId = clientOrderId || crypto.randomUUID();
      const unitPrice = Number(listing.price); const deliveryFee = listing.delivery_fee == null ? 0 : Number(listing.delivery_fee); const totalAmount = (unitPrice * quantity) + deliveryFee;
    ...
    } catch (error) { try { await client.query('ROLLBACK'); } catch {} if (error?.code === '23505') return res.status(409).json({ error: 'order already exists' }); console.error('marketplace order create', error); return res.status(500).json({ error: 'protected order creation failed' }); }
    finally { client.release(); }
  });

  app.get('/api/marketplace/orders', auth, async (req, res) => {
    try { await ensureMarketplaceTransactionSchema(); const result = await pool.query(`SELECT o.* FROM (SELECT o.* FROM marketplace_orders o WHERE o.buyer_id = $1 ORDER BY o.created_at DESC LIMIT 100) o UNION ALL SELECT o.* FROM (SELECT o.* FROM marketplace_orders o WHERE o.seller_id = $1 ORDER BY o.created_at DESC LIMIT 100) o ORDER BY created_at DESC LIMIT 100`, [req.user.sub]); return res.json({ orders: result.rows.map(publicOrder) }); }
  ...
  app.get('/api/marketplace/orders/:id', auth, async (req, res) => {
    ...
    try {
      ...
      const result = await pool.query(`SELECT o.* FROM marketplace_orders o WHERE o.id=$1`, [id]); const order = result.rows[0];
      if (!order) return res.status(404).json({ error: 'order not found' }); if (!isParticipant(order, req.user.sub)) return res.status(403).json({ error: 'order unavailable' });
