export function registerSocialRoutes({ app, pool, auth, findUserByUsername }) {
  ...
  const ensureSocialSchema = async () => {
    if (!socialSchemaPromise) socialSchemaPromise = pool.query(`
      CREATE TABLE IF NOT EXISTS social_posts (id BIGSERIAL PRIMARY KEY, author_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE, text TEXT NOT NULL DEFAULT '', visibility TEXT NOT NULL DEFAULT 'PUBLIC' CHECK (visibility IN ('PUBLIC','FRIENDS_ONLY','SELECTED_PEOPLE','ONLY_ME')), media_id BIGINT REFERENCES message_media(id) ON DELETE SET NULL, media_type TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW());
