...
async function ensureAdvertisingSchema() {
  ...
  await pool.query(`
    ...
    ALTER TABLE marketplace_ad_campaigns ADD COLUMN IF NOT EXISTS review_note TEXT;
    ALTER TABLE marketplace_ad_campaigns ADD COLUMN IF NOT EXISTS approved_at TIMESTAMPTZ;
    ALTER TABLE marketplace_ad_campaigns ADD COLUMN IF NOT EXISTS payment_status TEXT;
    ALTER TABLE marketplace_ad_campaigns ADD COLUMN IF NOT EXISTS payment_reference TEXT;
    ALTER TABLE marketplace_ad_campaigns ADD COLUMN IF NOT EXISTS payment_amount_kobo BIGINT;
    ...
    CREATE INDEX IF NOT EXISTS marketplace_ad_campaigns_payment_reference_idx ON marketplace_ad_campaigns(payment_reference);
...
function parseTargeting(value) {
  ...
  if (result.ageMin !== undefined && result.ageMax !== undefined && result.ageMin > result.ageMax) return null;
  return result;
}
...
export function registerMarketplaceAdvertisingRoutes({ app }) {
  ...
  app.get("/api/advertising/dashboard", async (req, res) => {
    ...
    try {
      const result = await pool.query(`SELECT
        ...
        COALESCE(SUM(c.spent_kobo),0)::bigint AS spent_kobo,
        COALESCE(SUM(c.total_budget_kobo),0)::bigint AS budget_kobo,
        ...
        FROM marketplace_ad_campaigns c LEFT JOIN marketplace_ad_metrics m ON m.campaign_id=c.id WHERE c.owner_id=$1`, [auth.userId]);
  ...
  app.patch("/api/advertising/campaigns/:id/status", async (req, res) => {
    ...
    try {
      ...
      if (status === "active" && (!current.rows[0].approved_at || current.rows[0].payment_status !== "paid")) {
        await client.query("ROLLBACK");
        return res.status(409).json({ error: !current.rows[0].approved_at ? "campaign approval required before activation" : "advertising payment required before activation" });
      }
  ...
  // Metrics ingestion is deliberately limited to active campaigns. Billing/spend remains server-owned.
