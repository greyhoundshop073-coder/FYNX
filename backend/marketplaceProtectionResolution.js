  ...
  try {
    ...
    const disputes = await pool.query(`
      ...
      WHERE d.status IN ('OPEN','UNDER_REVIEW')
        AND NOT EXISTS (SELECT 1 FROM marketplace_protection_cases c WHERE c.dispute_id=d.id)
    ...
    for (const dispute of disputes.rows) {
      ...
      try {
        ...
        const inserted = await client.query(`
          INSERT INTO marketplace_protection_cases (id,order_id,opened_by,role,case_type,reason,details,status,idempotency_key,dispute_id)
          VALUES ($1,$2,$3,$4,'DISPUTE',$5,$6,CASE WHEN $7 IN ('OPEN','UNDER_REVIEW') THEN $7 ELSE 'OPEN' END,$8,$9)
          ON CONFLICT (idempotency_key) DO NOTHING
          RETURNING id
        `, [caseId, dispute.order_id, dispute.opened_by, role, String(dispute.reason), String(dispute.details || ''), dispute.status, key, dispute.id]);
        if (inserted.rows[0]) {
          await client.query(`
            INSERT INTO marketplace_protection_audit (case_id,order_id,actor_id,action,metadata)
            VALUES ($1,$2,$3,'DISPUTE_LINKED',$4::jsonb)
          `, [inserted.rows[0].id, dispute.order_id, dispute.opened_by, JSON.stringify({ disputeId: String(dispute.id), reason: dispute.reason })]);
        }
  ...
  app.get('/api/admin/marketplace/protection/cases', auth, async (req, res) => {
    try {
      ...
      const result = await pool.query(`
        SELECT c.id,c.order_id,c.dispute_id,c.opened_by,c.role,c.case_type,c.reason,c.details,c.status,c.created_at,c.updated_at,
      ...
      try {
        ...
        if (resolution === 'BUYER') {
          ...
          if (existing?.status === 'SUCCEEDED') {
            ...
            if (row.dispute_id) await client.query(`UPDATE marketplace_order_disputes SET status='RESOLVED_BUYER',resolution_notes=$1,updated_at=NOW() WHERE id=$2`, [note, row.dispute_id]);
            await client.query('COMMIT');
          ...
          if (row.dispute_id) {
            const disputeStatus = resolution === 'SELLER' ? 'RESOLVED_SELLER' : 'CANCELLED';
            await client.query(`UPDATE marketplace_order_disputes SET status=$1,resolution_notes=$2,updated_at=NOW() WHERE id=$3`, [disputeStatus, note, row.dispute_id]);
          }
      ...
      if (refundJob) {
        try {
          ...
          try {
            ...
            if (refundJob.disputeId) await finishClient.query(`UPDATE marketplace_order_disputes SET status='RESOLVED_BUYER',resolution_notes=$1,updated_at=NOW() WHERE id=$2`, [note, refundJob.disputeId]);
            ...
            await finishClient.query(`UPDATE marketplace_listings SET reserved_quantity=GREATEST(0,reserved_quantity-$1),updated_at=NOW() WHERE id=$2`, [refundJob.quantity, refundJob.listingId]);
            const escrow = (await finishClient.query('SELECT id,amount,currency FROM marketplace_escrows WHERE order_id=$1', [refundJob.orderId])).rows[0];
            if (escrow) {
              await finishClient.query(`INSERT INTO marketplace_ledger_entries (escrow_id,order_id,account,entry_type,amount,currency,idempotency_key,metadata) VALUES ($1,$2,'buyer_refund','REFUND',$3,$4,$5,$6::jsonb) ON CONFLICT (idempotency_key) DO NOTHING`, [escrow.id, refundJob.orderId, escrow.amount, escrow.currency, `REFUND-${refundJob.orderId}`, JSON.stringify({ provider: 'paystack', reference: refundJob.reference, caseId: refundJob.caseId })]);
            }
