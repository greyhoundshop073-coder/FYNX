...
export function registerMarketplaceCompletionRoutes({ app, pool, auth }) {
  ...
  app.get('/api/marketplace/seller/orders', auth, async (req, res) => {
    try {
      ...
      console.error('marketplace seller orders', error);
      return res.status(500).json({ error: 'seller order lookup failed' });
    }
  });
  ...
  app.post('/api/marketplace/orders/:id/fulfillment', auth, async (req, res) => {
    ...
    const address = req.body?.shippingAddress && typeof req.body.shippingAddress === 'object' && !Array.isArray(req.body.shippingAddress)
    ...
    if (method === 'DELIVERY') {
      if (!address || typeof address.name !== 'string' || typeof address.phone !== 'string' || typeof address.address !== 'string' || !address.address.trim()) {
    ...
    try {
      ...
      const safeAddress = method === 'DELIVERY' ? {
        ...
        address: String(address.address).trim().slice(0, 500),
        city: typeof address.city === 'string' ? address.city.trim().slice(0, 100) : '',
        state: typeof address.state === 'string' ? address.state.trim().slice(0, 100) : '',
  ...
  app.post('/api/marketplace/orders/:id/ship', auth, async (req, res) => {
    ...
    try {
      ...
      try { await client.query('ROLLBACK'); } catch {}
      console.error('marketplace ship', error);
      return res.status(500).json({ error: 'order shipping failed' });
    } finally { client.release(); }
  });
  ...
  app.post('/api/marketplace/orders/:id/confirm-delivery', auth, async (req, res) => {
    ...
    try {
      ...
      try { await client.query('ROLLBACK'); } catch {}
      console.error('marketplace confirm delivery', error);
  ...
  app.post('/api/marketplace/orders/:id/complete', auth, async (req, res) => {
    ...
    try {
      ...
      try { await client.query('ROLLBACK'); } catch {}
      console.error('marketplace complete', error);
      return res.status(500).json({ error: 'order completion failed' });
    } finally { client.release(); }
  });
}
