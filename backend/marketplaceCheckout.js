...
export function registerMarketplaceCheckoutRoutes({ app, pool, auth }) {
  ...
  const normalizeMethod = (value) => {
    ...
    return method === 'DELIVERY' || method === 'PICKUP' ? method : null;
  };
  ...
  const normalizeAddress = (value) => {
    ...
    return {
      ...
      state: typeof value.state === 'string' ? value.state.trim().slice(0, 100) : '',
      country: typeof value.country === 'string' ? value.country.trim().slice(0, 100) : ''
    };
  };
  ...
  app.post('/api/marketplace/checkout/quote', auth, async (req, res) => {
    ...
    const fulfillmentMethod = normalizeMethod(req.body?.fulfillmentMethod);
    ...
    try {
      ...
      if (quantity > available) {
        await client.query('ROLLBACK');
        return res.status(409).json({ error: 'requested quantity is not available', availableQuantity: Math.max(0, available) });
      }
      ...
      const deliveryFee = fulfillmentMethod === 'DELIVERY' ? Math.max(0, Number(listing.delivery_fee || 0)) : 0;
      const config = feeConfig();
      const rawFee = Math.round((productSubtotal * (config.bps / 10000) + config.fixed) * 100) / 100;
      const marketplaceFee = Math.max(0, rawFee);
      const buyerShare = config.mode === 'BUYER' ? 1 : config.mode === 'SELLER' ? 0 : config.mode === 'SPLIT' ? config.buyerShare / 10000 : 0;
      const marketplaceFeeBuyer = Math.round(marketplaceFee * buyerShare * 100) / 100;
      const marketplaceFeeSeller = Math.round((marketplaceFee - marketplaceFeeBuyer) * 100) / 100;
      const discountAmount = 0;
      const buyerTotal = Math.round((productSubtotal + deliveryFee + marketplaceFeeBuyer - discountAmount) * 100) / 100;
      ...
      if (!(buyerTotal > 0) || sellerNetAmount < 0) {
        await client.query('ROLLBACK');
        return res.status(500).json({ error: 'checkout total calculation failed' });
      }
      ...
      return res.json({
        quote: {
          ...
          listingUpdatedAt: listing.updated_at || null,
          ...
          discount: { amount: 0, code: null, status: 'NOT_APPLIED' },
          protection: { payment: 'PROTECTED', payout: 'RELEASED_AFTER_BUYER_CONFIRMATION' }
        }
      });
    } catch (error) {
      try { await client.query('ROLLBACK'); } catch {}
      console.error('marketplace checkout quote', error);
      return res.status(500).json({ error: 'checkout quote could not be prepared' });
    } finally {
      client.release();
    }
  });
