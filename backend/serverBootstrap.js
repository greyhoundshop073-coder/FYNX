...
source = source.replace(
  ...
  'wss.on("connection", (socket, req) => {\n  // R3 bridge is attached after authentication by the production bootstrap patch.\n  if (typeof fynxRealtimeMessagingAttach === "function") fynxRealtimeMessagingAttach(socket, String(jwt.decode(new URL(req.url || "/realtime", `http://${req.headers.host || "localhost"}`).searchParams.get("token") || "")?.sub || ""));'
);
