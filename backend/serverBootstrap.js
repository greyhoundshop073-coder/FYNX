...
// The message limiter is mounted before route-level auth in the legacy server.
// Keep that ordering for compatibility, but derive the authenticated subject
// inside the limiter so one authenticated user cannot share the anonymous bucket.
source = source.replace(
  'function rateLimit(bucket, limit) {\n  return (req, res, next) => {\n    const key = `${bucket}:${req.ip || req.socket.remoteAddress || "unknown"}:${req.user?.sub || "anonymous"}`;',
  'function rateLimit(bucket, limit) {\n  return (req, res, next) => {\n    let authenticatedSubject = req.user?.sub || "";\n    if (!authenticatedSubject && JWT_SECRET) {\n      const header = req.get("authorization") || "";\n      const token = header.startsWith("Bearer ") ? header.slice(7).trim() : "";\n      if (token) {\n        try { authenticatedSubject = String(jwt.verify(token, JWT_SECRET).sub || ""); } catch { authenticatedSubject = ""; }\n      }\n    }\n    const identity = authenticatedSubject ? `user:${authenticatedSubject}` : `ip:${req.ip || req.socket.remoteAddress || "unknown"}`;\n    const key = `${bucket}:${identity}`;'
);
...
// R3 private-message realtime helpers are defined in the production runtime,
// but attached only after the existing WebSocket handler has successfully
// verified the JWT. No unverified token data is used to establish identity.
const realtimeMessagingPatch = `
\nconst fynxRealtimeMessagingMaxPacketBytes = 64 * 1024;\nconst fynxRealtimeMessagingValidUserId = (value) => typeof value === "string" && /^\\d+$/.test(value) && value.length <= 20;\nconst fynxRealtimeMessagingBlocks = async (a, b) => {\n  if (!pool) return true;\n  const result = await pool.query(\"SELECT 1 FROM blocks WHERE (blocker_id=$1 AND blocked_id=$2) OR (blocker_id=$2 AND blocked_id=$1) LIMIT 1\", [a, b]);\n  return Boolean(result.rowCount);\n};\nconst fynxRealtimeMessagingCanSend = async (senderId, targetId) => {\n  if (!pool || !fynxRealtimeMessagingValidUserId(targetId) || targetId === senderId) return false;\n  if (await fynxRealtimeMessagingBlocks(senderId, targetId)) return false;\n  const result = await pool.query(\"SELECT u.id FROM users u WHERE u.id=$1 AND (COALESCE(u.messages_visibility, 'EVERYONE')='EVERYONE' OR (COALESCE(u.messages_visibility, 'EVERYONE')='MY_FRIENDS' AND EXISTS (SELECT 1 FROM friendships f WHERE ((f.requester_id=$1 AND f.addressee_id=$2) OR (f.requester_id=$2 AND f.addressee_id=$1)) AND f.status='ACCEPTED')) LIMIT 1\", [targetId, senderId]);\n  return Boolean(result.rowCount);\n};\nconst fynxRealtimeMessagingTypingTarget = async (senderId, targetId) => {\n  return fynxRealtimeMessagingCanSend(senderId, targetId);\n};\nconst fynxRealtimeMessagingRead = async (userId, ids) => {\n  if (!pool || !ids.length) return;\n  const result = await pool.query(\"UPDATE messages SET read_at=NOW(), delivered_at=COALESCE(delivered_at,NOW()) WHERE id=ANY($1::bigint[]) AND recipient_id=$2 AND deleted=FALSE RETURNING id,sender_id\", [ids, userId]);\n  for (const row of result.rows) broadcastToUser(row.sender_id, { type: \"message_status\", messageId: String(row.id), status: \"read\" });\n};\nconst fynxRealtimeMessagingAck = async (userId, id) => {\n  if (!pool || !Number.isSafeInteger(id) || id < 1) return;\n  const result = await pool.query(\"UPDATE messages SET delivered_at=COALESCE(delivered_at,NOW()) WHERE id=$1 AND recipient_id=$2 AND deleted=FALSE RETURNING id,sender_id\", [id, userId]);\n  if (result.rows[0]) broadcastToUser(result.rows[0].sender_id, { type: \"message_status\", messageId: String(id), status: \"delivered\" });\n};\nconst fynxRealtimeMessagingRate = new Map();\nconst fynxRealtimeMessagingRateWindowMs = 60_000;\nconst fynxRealtimeMessagingRateLimit = 180;\nconst fynxRealtimeMessagingAllow = (userId) => {\n  const now = Date.now();\n  const current = fynxRealtimeMessagingRate.get(userId);\n  if (!current || now - current.startedAt >= fynxRealtimeMessagingRateWindowMs) {\n    fynxRealtimeMessagingRate.set(userId, { startedAt: now, count: 1 });\n    return true;\n  }\n  if (current.count >= fynxRealtimeMessagingRateLimit) return false;\n  current.count += 1;\n  return true;\n};\nsetInterval(() => {\n  const cutoff = Date.now() - fynxRealtimeMessagingRateWindowMs;\n  for (const [key, value] of fynxRealtimeMessagingRate) if (value.startedAt < cutoff) fynxRealtimeMessagingRate.delete(key);\n}, fynxRealtimeMessagingRateWindowMs).unref();\nconst fynxRealtimeMessagingAttach = (socket, userId) => {\n  socket.on(\"message\", (raw) => {\n    try {\n      const text = raw.toString();\n      if (Buffer.byteLength(text, \"utf8\") > fynxRealtimeMessagingMaxPacketBytes) return;\n      const body = JSON.parse(text);\n      if (!body || typeof body !== \"object\" || Array.isArray(body) || body.type === \"call\") return;\n      if (!fynxRealtimeMessagingAllow(userId)) return;\n      if (body.type === \"typing\") {\n        const targetId = String(body.toUserId || body.recipientId || \"\");\n        const isTyping = body.isTyping === true || body.isTyping === false ? body.isTyping : null;\n        if (isTyping == null) return;\n        fynxRealtimeMessagingTypingTarget(userId, targetId).then(valid => {\n          if (valid) broadcastToUser(targetId, { type: \"typing\", userId, isTyping });\n        }).catch(error => console.error(\"realtime typing\", error));\n        return;\n      }\n      if (body.type === \"message_ack\") {\n        const id = Number(body.messageId);\n        if (!Number.isSafeInteger(id) || id < 1) return;\n        fynxRealtimeMessagingAck(userId, id).catch(error => console.error(\"realtime message ack\", error));\n        return;\n      }\n      if (body.type === \"read\") {\n        if (!Array.isArray(body.messageIds)) return;\n        const ids = [...new Set(body.messageIds.map(Number).filter(id => Number.isSafeInteger(id) && id > 0))].slice(0, 100);\n        fynxRealtimeMessagingRead(userId, ids).catch(error => console.error(\"realtime message read\", error));\n      }\n    } catch {\n      // Ignore malformed non-call realtime packets. The authenticated socket remains alive.\n    }\n  });\n};\n\n`;
const realtimeMessagingMarker = 'registerSocialRoutes(app, { pool, auth, findUserByUsername });';
if (!source.includes('const fynxRealtimeMessagingMaxPacketBytes')) {
  if (!source.includes(realtimeMessagingMarker)) throw new Error("FYNX bootstrap could not locate realtime patch marker");
  source = source.replace(realtimeMessagingMarker, realtimeMessagingPatch + realtimeMessagingMarker);
}
source = source.replace(
  'wss.on("connection", (socket, req) => {',
  'wss.on("connection", (socket, req) => {'
);
const attachAfterAuthNeedle = '    const userId = String(user.sub);';
if (!source.includes(attachAfterAuthNeedle)) throw new Error("FYNX bootstrap could not locate authenticated realtime identity");
source = source.replace(
  attachAfterAuthNeedle,
  attachAfterAuthNeedle + '\n    fynxRealtimeMessagingAttach(socket, userId);'
);

// Enforce the same message-visibility policy on the authenticated HTTP send
// path that realtime typing already uses. Blocks remain checked by the route.
source = source.replace(
  ...
  '    const recipient = await findUserByUsername(recipientUsername);\n    if (!recipient || String(recipient.id) === String(req.user.sub)) return res.status(400).json({ error: "invalid recipient" });\n    if (!(await fynxRealtimeMessagingCanSend(String(req.user.sub), String(recipient.id)))) return res.status(403).json({ error: "conversation unavailable" });'
);
