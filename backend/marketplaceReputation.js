...
export function registerMarketplaceReputationRoutes({ app, pool, auth }) {
  ...
  const reputation = async (sellerUsername) => {
    ...
    const result = await pool.query(`
      WITH order_stats AS (
        SELECT u.id AS seller_id,u.username,u.display_name,
        ...
        FROM users u LEFT JOIN marketplace_orders o ON o.seller_id = u.id
        GROUP BY u.id, u.username, u.display_name
      ),
      review_stats AS (
        SELECT seller_id,COUNT(*)::INTEGER AS review_count,COALESCE(AVG(rating), 0)::NUMERIC(4,2) AS average_rating,
          COUNT(*) FILTER (WHERE rating >= 4)::INTEGER AS positive_reviews
        FROM marketplace_seller_reviews GROUP BY seller_id
      ),
      ranked AS (
        SELECT s.*,COALESCE(r.review_count, 0)::INTEGER AS review_count,COALESCE(r.average_rating, 0)::NUMERIC(4,2) AS average_rating,
          COALESCE(r.positive_reviews, 0)::INTEGER AS positive_reviews,
          RANK() OVER (ORDER BY s.successful_orders DESC,CASE WHEN s.total_orders > 0 THEN s.successful_orders::NUMERIC / s.total_orders ELSE 0 END DESC,s.seller_id ASC)::INTEGER AS seller_rank,
          COUNT(*) OVER ()::INTEGER AS seller_count
        FROM order_stats s LEFT JOIN review_stats r ON r.seller_id = s.seller_id
      ) SELECT * FROM ranked WHERE username = $1
    `, [sellerUsername]);
    const row = result.rows[0];
    if (!row) return null;
    const successful = Number(row.successful_orders || 0), total = Number(row.total_orders || 0), successful30d = Number(row.successful_orders_30d || 0), total30d = Number(row.total_orders_30d || 0), reviews = Number(row.review_count || 0), positive = Number(row.positive_reviews || 0);
    const completionRate = total > 0 ? (successful / total) * 100 : 0, completionRate30d = total30d > 0 ? (successful30d / total30d) * 100 : 0, positiveRating = reviews > 0 ? (positive / reviews) * 100 : 0;
    let tier = 'NEW SELLER';
    ...
    return { sellerId: String(row.seller_id),username: row.username,displayName: row.display_name,rank: Number(row.seller_rank),sellerCount: Number(row.seller_count),successfulSales: successful,totalOrders: total,completionRate: Number(completionRate.toFixed(2)),successfulSales30d: successful30d,totalOrders30d: total30d,completionRate30d: Number(completionRate30d.toFixed(2)),reviewCount: reviews,averageRating: Number(Number(row.average_rating || 0).toFixed(2)),positiveRating: Number(positiveRating.toFixed(2)),tier };
  };
  ...
  app.get('/api/marketplace/sellers/:username/reputation', auth, async (req, res) => {
    ...
    } catch (error) { console.error('marketplace seller reputation', error); return res.status(500).json({ error: 'seller reputation lookup failed' }); }
  });
  ...
  app.post('/api/marketplace/orders/:id/review', auth, async (req, res) => {
    try {
      ...
      const result = await pool.query(`INSERT INTO marketplace_seller_reviews (id, order_id, seller_id, buyer_id, rating, comment) VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT (order_id, buyer_id) DO NOTHING RETURNING id, rating, comment, created_at`, [id, order.id, order.seller_id, order.buyer_id, rating, comment]);
    ...
    } catch (error) { console.error('marketplace seller review', error); return res.status(500).json({ error: 'seller review failed' }); }
  });
  ...
  const paystackRequest = async (path, options = {}) => {
    ...
    const response = await fetch(`https://api.paystack.co${path}`, { ...options, headers: { Authorization: `Bearer ${secret}`, 'Content-Type': 'application/json', ...(options.headers || {}) } });
  ...
  const amountSubunit = (amount, currency) => { const normalized = String(currency || '').trim().toUpperCase(); if (!['NGN', 'USD'].includes(normalized)) return null; const value = Number(amount); if (!Number.isFinite(value) || value <= 0) return null; return Math.round(value * 100); };

  app.post('/api/marketplace/orders/:id/payment', auth, async (req, res) => {
    const orderId = parseOrderId(req.params.id), email = typeof req.body?.email === 'string' ? req.body.email.trim().toLowerCase() : '';
    if (!orderId || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return res.status(400).json({ error: 'valid order id and customer email are required' });
    try {
      await ensureSchema();
      const existingResult = await pool.query(`SELECT id,buyer_id,total_amount,currency,status,payment_reference,payment_authorization_url,payment_access_code FROM marketplace_orders WHERE id=$1`, [orderId]);
      ...
      try {
        ...
        if (locked.payment_reference && locked.payment_authorization_url) { await client.query('COMMIT'); return res.status(200).json({ orderId: String(locked.id), reference: locked.payment_reference, authorizationUrl: locked.payment_authorization_url, accessCode: locked.payment_access_code || null, idempotent: true, amountSubunit: amount, currency: String(existing.currency).toUpperCase() }); }
        ...
        if (!updated.rows[0]) { const current = (await client.query(`SELECT payment_reference,payment_authorization_url,payment_access_code FROM marketplace_orders WHERE id=$1`, [orderId])).rows[0]; await client.query('COMMIT'); if (current?.payment_reference && current?.payment_authorization_url) return res.status(200).json({ orderId, reference: current.payment_reference, authorizationUrl: current.payment_authorization_url, accessCode: current.payment_access_code || null, idempotent: true, amountSubunit: amount, currency: String(existing.currency).toUpperCase() }); return res.status(409).json({ error: 'payment initialization could not be committed; retry the order payment request' }); }
      ...
      console.error('marketplace payment initialize', error); return res.status(502).json({ error: 'payment initialization failed' });
    }
  });
  ...
  app.get('/api/marketplace/payments/verify/:reference', auth, async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        if (error?.code === 'PAYMENT_ORDER_NOT_FOUND') return res.status(404).json({ error: 'order not found' });
        if (error?.code === 'PAYMENT_DATA_MISMATCH' || error?.code === 'PAYMENT_STATE_CONFLICT') return res.status(409).json({ error: error.message });
        throw error;
      } finally { client.release(); }
    } catch (error) {
      if (error?.code === 'PAYSTACK_NOT_CONFIGURED') return res.status(503).json({ error: 'marketplace payments are not configured yet' });
      console.error('marketplace payment verify', error); return res.status(502).json({ error: 'payment verification failed' });
    }
  });
}
