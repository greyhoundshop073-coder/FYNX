...
export function registerGroupRoutes({ app }) {
  ...
  const getMessage = async (groupId,messageId) => (await pool.query(`SELECT m.*,gm.role FROM fynx_group_messages m JOIN fynx_group_members gm ON gm.group_id=m.group_id AND gm.user_id=$3 WHERE m.id=$1 AND m.group_id=$2 LIMIT 1`,[messageId,groupId])).rows[0] || null;
