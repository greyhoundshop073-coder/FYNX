...
private fun encode(events: List<FynxCalendarEvent>): String = events.joinToString("\\n") { listOf(it.id, it.title, it.date, it.time, it.notes).joinToString("|") }
private fun decode(raw: String): List<FynxCalendarEvent> = raw.lineSequence().mapNotNull { line ->
    val p = line.split("|", limit = 5)
    if (p.size == 5) p[0].toLongOrNull()?.let { FynxCalendarEvent(it, p[1], p[2], p[3], p[4]) } else null
}.toList()
...
fun CalendarPanel() {
    ...
    Column(Modifier.fillMaxSize()) {
        ...
        Column {
            cells.chunked(7).forEach { week -> Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceEvenly) {
                week.forEach { day -> if (day == null) Spacer(Modifier.size(40.dp)) else {
        ...
        Row(Modifier.fillMaxWidth()) {
            OutlinedTextField(time, { time = it }, label = { Text("Time") }, placeholder = { Text("e.g. 14:30") }, singleLine = true, modifier = Modifier.weight(1f))
            Spacer(Modifier.width(8.dp))
            Button(onClick = { val clean = title.trim(); if (clean.isNotEmpty()) { val event = FynxCalendarEvent(nextId++, clean, selectedDate, time.trim(), notes.trim()); events = events + event; saveEvents(context, events); title = ""; time = ""; notes = "" } }) { Text("Add") }
        }
        OutlinedTextField(notes, { notes = it }, label = { Text("Details / notes") }, modifier = Modifier.fillMaxWidth(), minLines = 2)
        Spacer(Modifier.height(8.dp))
        LazyColumn(verticalArrangement = Arrangement.spacedBy(6.dp)) {
            items(events.filter { it.date == selectedDate }, key = { it.id }) { event -> Card(Modifier.fillMaxWidth()) {
                Column(Modifier.padding(10.dp)) {
                    Text(event.title, style = MaterialTheme.typography.titleMedium)
                    if (event.time.isNotBlank()) Text(event.time, style = MaterialTheme.typography.labelMedium)
                    if (event.notes.isNotBlank()) Text(event.notes, style = MaterialTheme.typography.bodyMedium)
                    Row { TextButton(onClick = { editing = event }) { Text("Edit") }; TextButton(onClick = { events = events.filterNot { it.id == event.id }; saveEvents(context, events) }) { Text("Delete") } }
                }
            }}
        }
    }
...
private fun EventEditor(event: FynxCalendarEvent, onCancel: () -> Unit, onSave: (FynxCalendarEvent) -> Unit) {
    ...
    Column(Modifier.fillMaxSize()) {
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) { TextButton(onClick = onCancel) { Text("Cancel") }; Text("Edit event", style = MaterialTheme.typography.titleLarge); TextButton(onClick = { onSave(event.copy(title = title.trim().ifEmpty { event.title }, date = date.trim().ifEmpty { event.date }, time = time.trim(), notes = notes.trim())) }) { Text("Save") } }
        HorizontalDivider(); Spacer(Modifier.height(16.dp))
        OutlinedTextField(title, { title = it }, label = { Text("Event title") }, singleLine = true, modifier = Modifier.fillMaxWidth())
        OutlinedTextField(date, { date = it }, label = { Text("Date") }, placeholder = { Text("yyyy-MM-dd") }, singleLine = true, modifier = Modifier.fillMaxWidth())
        OutlinedTextField(time, { time = it }, label = { Text("Time") }, placeholder = { Text("e.g. 14:30") }, singleLine = true, modifier = Modifier.fillMaxWidth())
        OutlinedTextField(notes, { notes = it }, label = { Text("Details / notes") }, modifier = Modifier.fillMaxWidth(), minLines = 3)
    }
}
