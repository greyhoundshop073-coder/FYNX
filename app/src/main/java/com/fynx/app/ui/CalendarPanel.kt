...
fun CalendarPanel() {
    ...
    Column(Modifier.fillMaxSize()) {
        ...
        Row(Modifier.fillMaxWidth()) {
            ...
            Button(onClick = { val clean = title.trim(); if (clean.isNotEmpty()) { events = events + FynxCalendarEvent(nextId++, clean, selectedDate, time.trim(), notes.trim()); title = ""; time = ""; notes = "" } }) { Text("Add") }
        }
            ...
            items(events.filter { it.date == selectedDate }, key = { it.id }) { event ->
                Card(Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(10.dp)) {
                        ...
                        Row {
                            TextButton(onClick = { editing = event }) { Text("Edit") }
                            TextButton(onClick = { events = events.filterNot { it.id == event.id } }) { Text("Delete") }
                        }
                    }
                }
            }
...
private fun EventEditor(event: FynxCalendarEvent, onCancel: () -> Unit, onSave: (FynxCalendarEvent) -> Unit) {
    ...
    Column(Modifier.fillMaxSize()) {
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
            ...
            TextButton(onClick = { onSave(event.copy(title = title.trim().ifEmpty { event.title }, date = date.trim().ifEmpty { event.date }, time = time.trim(), notes = notes.trim())) }) { Text("Save") }
        }
