...
object FynxHomePostStore {
    ...
    private fun accountKey(context: Context): String = (FynxAuthStore.storedUsername(context) ?: "@preview").trim().lowercase().removePrefix("@").ifBlank { "preview" }
    ...
    fun load(context: Context): List<FynxPost> = runCatching {
        ...
        buildList {
            for (index in 0 until array.length()) {
                ...
                if (id.isNotBlank() && author.isNotBlank() && (text.isNotBlank() || mediaUri != null)) add(FynxPost(id, author, text, item.optLong("timestamp", 0L), runCatching { FynxPostVisibility.valueOf(item.optString("visibility")) }.getOrDefault(FynxPostVisibility.PUBLIC), mediaUri, item.optInt("likeCount", 0).coerceAtLeast(0), item.optInt("commentCount", 0).coerceAtLeast(0), item.optBoolean("likedByCurrentUser"), item.optBoolean("savedByCurrentUser")))
            }
    ...
    fun loadComments(context: Context, postId: String): List<FynxPostComment> = runCatching {
        ...
        val array = JSONArray(raw)
        buildList { for (index in 0 until array.length()) { val item = array.optJSONObject(index) ?: continue; if (item.optString("postId") == postId && item.optString("text").isNotBlank()) add(FynxPostComment(item.optString("id"), postId, item.optString("authorUsername"), item.optString("text"), item.optLong("timestamp", 0L))) } }.sortedBy { it.timestamp }
    }.getOrElse { emptyList() }

    private fun loadAllComments(context: Context): List<FynxPostComment> = runCatching {
        val raw = context.getSharedPreferences(COMMENTS_PREFS, Context.MODE_PRIVATE).getString(commentsKey(context), null) ?: return emptyList()
        val array = JSONArray(raw); buildList { for (i in 0 until array.length()) { val x = array.optJSONObject(i) ?: continue; add(FynxPostComment(x.optString("id"), x.optString("postId"), x.optString("authorUsername"), x.optString("text"), x.optLong("timestamp", 0L))) } }
    }.getOrElse { emptyList() }

    private fun save(context: Context, posts: List<FynxPost>) {
        val array = JSONArray(); posts.sortedByDescending { it.timestamp }.take(MAX_POSTS).forEach { post -> array.put(JSONObject().apply { put("id", post.id); put("authorUsername", post.authorUsername); put("text", post.text); put("timestamp", post.timestamp); put("visibility", post.visibility.name); put("mediaUri", post.mediaUri ?: ""); put("likeCount", post.likeCount.coerceAtLeast(0)); put("commentCount", post.commentCount.coerceAtLeast(0)); put("likedByCurrentUser", post.likedByCurrentUser); put("savedByCurrentUser", post.savedByCurrentUser) }) }
    ...
    private fun saveComments(context: Context, comments: List<FynxPostComment>) {
        val array = JSONArray(); comments.takeLast(MAX_COMMENTS).forEach { c -> array.put(JSONObject().apply { put("id", c.id); put("postId", c.postId); put("authorUsername", c.authorUsername); put("text", c.text); put("timestamp", c.timestamp) }) }
    ...
    /** Compatibility overload for existing callers; new UI should pass the actual comment text. */
    fun addComment(context: Context, postId: String) = save(context, load(context).map { if (it.id == postId) it.copy(commentCount = it.commentCount + 1) else it })
