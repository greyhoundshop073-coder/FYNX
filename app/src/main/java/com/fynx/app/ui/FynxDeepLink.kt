...
object FynxDeepLinkParser {
    ...
    fun inviteWebLink(code: String?, groupId: String? = null): String {
        ...
        return Uri.Builder().scheme("https").authority(FYNX_HOST).path(INVITE_PATH)
            ...
            .apply { groupId?.trim()?.takeIf { it.isNotBlank() }?.let { appendQueryParameter("groupId", it) } }
            .build().toString()
    }
        ...
        if (isFynxScheme) {
            return when (host) {
                ...
                "group" -> when {
                    ...
                    normalizedPath.size == 3 && normalizedPath[1].equals("invite", true) -> cleanIdentifier(normalizedPath[2])?.let { FynxDeepLinkDestination.Invite(it, value) }
                    else -> null
                }
                "marketplace" -> if (normalizedPath.size <= 1) FynxDeepLinkDestination.Marketplace(value) else null
                "invite" -> if (normalizedPath.size <= 1) FynxDeepLinkDestination.Invite(value ?: queryCode, queryGroupId) else null
                else -> null
            }
        }
        return when (first) {
