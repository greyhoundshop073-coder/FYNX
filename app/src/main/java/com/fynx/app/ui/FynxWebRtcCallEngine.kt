...
class FynxWebRtcCallEngine(
    ...
    override fun connect(session: FynxCallSession) {
        disconnect(); remoteDescriptionSet = false; pendingRemoteCandidates.clear(); iceRestartInFlight = false; iceRecoveryAttempts = 0; audioRouter.start(session.type == FynxCallType.VIDEO)
        peerConnection = factory.createPeerConnection(PeerConnection.RTCConfiguration(iceServers), object : PeerConnection.Observer {
            ...
            override fun onAddStream(stream: MediaStream) { stream.audioTracks.firstOrNull()?.let(callbacks.onRemoteAudioTrack); stream.videoTracks.firstOrNull()?.let(callbacks.onRemoteVideoTrack) }
            override fun onRemoveStream(stream: MediaStream) = Unit
            override fun onDataChannel(channel: org.webrtc.DataChannel) = Unit
            override fun onRenegotiationNeeded() = Unit
            override fun onAddTrack(receiver: org.webrtc.RtpReceiver, mediaStreams: Array<out MediaStream>) { when (val track = receiver.track()) { is AudioTrack -> callbacks.onRemoteAudioTrack(track); is VideoTrack -> callbacks.onRemoteVideoTrack(track) } }
        })
        createLocalAudio(); if (session.type == FynxCallType.VIDEO) createLocalVideo()
    }
    private fun scheduleIceRecovery() {
        if (iceRestartInFlight || recoveryJob?.isActive == true || iceRecoveryAttempts >= 3) return
        recoveryJob = recoveryScope.launch {
            delay(if (iceRecoveryAttempts == 0) 1500L else 5000L)
            ...
            delay(8000L)
            if (peerConnection === pc && pc.iceConnectionState() != PeerConnection.IceConnectionState.CONNECTED && pc.iceConnectionState() != PeerConnection.IceConnectionState.COMPLETED) {
                iceRestartInFlight = false
                scheduleIceRecovery()
            }
        }
    ...
    private fun createLocalVideo() {
        val enumerator = Camera2Enumerator(appContext); val deviceName = enumerator.deviceNames.firstOrNull { enumerator.isFrontFacing(it) } ?: enumerator.deviceNames.firstOrNull() ?: return
        cameraCapturer = enumerator.createCapturer(deviceName, FynxCallCameraEvents(callbacks.onError)); videoSource = factory.createVideoSource(false); videoTrack = factory.createVideoTrack("fynx-video", videoSource); videoTrack?.setEnabled(true)
        videoTrack?.let { peerConnection?.addTrack(it); callbacks.onLocalVideoTrack(it) }
        eglBase = org.webrtc.EglBase.create(); surfaceTextureHelper = SurfaceTextureHelper.create("FYNX-Camera", eglBase!!.eglBaseContext); cameraCapturer?.initialize(surfaceTextureHelper, appContext, videoSource?.capturerObserver); cameraCapturer?.startCapture(1280, 720, 30)
    }
