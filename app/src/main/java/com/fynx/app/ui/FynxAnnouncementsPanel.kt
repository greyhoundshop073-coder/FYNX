...
@Composable
fun FynxAnnouncementsPanel(onBack: () -> Unit = {}) {
    ...
    Column(Modifier.fillMaxSize().padding(16.dp)) {
        ...
        when {
            ...
            error != null -> Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(16.dp)) { Text("Could not load announcements"); Spacer(Modifier.height(6.dp)); Text(error!!, color = MaterialTheme.colorScheme.error); Spacer(Modifier.height(10.dp)); TextButton(onClick = { loading = true; error = null }) { Text("Retry") } } }
    ...
    Column(Modifier.fillMaxSize().padding(16.dp)) {
        ...
        if (loading) CircularProgressIndicator()
            ...
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                StatCard("Users", d.users); StatCard("Reports", d.openReports)
            }
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                StatCard("Appeals", d.openAppeals); StatCard("Safety / 24h", d.safetyEvents24h)
            }
            ...
            Button(enabled = title.isNotBlank() && body.isNotBlank(), onClick = {
                ...
                LaunchedEffectKey.launch(context, title, body) { result -> statusMessage = result }
            }) { Text("Publish") }
...
private object LaunchedEffectKey {
    fun launch(context: Context, title: String, body: String, callback: (String) -> Unit) {
        kotlinx.coroutines.CoroutineScope(kotlinx.coroutines.Dispatchers.Main).launch {
            FynxAdminClient.publishAnnouncement(context, title, body, "NORMAL").onSuccess { callback("Announcement published.") }.onFailure { callback(it.message ?: "Publish failed.") }
        }
    }
}
