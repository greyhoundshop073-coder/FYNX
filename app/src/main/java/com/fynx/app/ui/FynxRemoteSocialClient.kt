...
object FynxRemoteSocialClient {
                ...
                for (i in 0 until array.length()) {
                    ...
                    add(
                        RemotePost(
                            ...
                            timestamp = o.optDouble("timestamp", 0.0).toLong(),
                            likeCount = o.optInt("likeCount"),
                            commentCount = o.optInt("commentCount"),
                            likedByCurrentUser = o.optBoolean("likedByCurrentUser"),
    ...
    suspend fun listings(
        ...
        FynxBackendClient.get(
            context,
            "/api/marketplace/listings?q=${URLEncoder.encode(query, "UTF-8")}" +
                "&category=${URLEncoder.encode(category, "UTF-8")}" +
                "&seller=${URLEncoder.encode(seller.removePrefix("@"), "UTF-8")}"
        ).mapCatching(::parseListings)

    suspend fun myListings(context: Context): Result<List<MarketplaceListing>> =
        FynxBackendClient.get(context, "/api/marketplace/my-listings")
            .mapCatching(::parseListings)
    ...
    suspend fun deleteMarketplaceListing(context: Context, id: String): Result<Unit> {
        ...
        return FynxBackendClient.delete(
            context,
            "/api/marketplace/listings/$numericId"
        ).map { Unit }
    }
    ...
    suspend fun createMarketplaceOrder(
        ...
        return FynxBackendClient.postJson(
            ...
            parseOrder(JSONObject(raw).getJSONObject("order"))
        }
    }
