...
object FynxRemoteSocialClient {
    ...
    suspend fun createMarketplaceAd(
        context: Context,
        listingId: String,
        title: String,
        description: String,
    ...
    suspend fun listings(
        ...
        FynxBackendClient.get(
            context,
            "/api/marketplace/listings?q=${URLEncoder.encode(query, "UTF-8")}&category=${URLEncoder.encode(category, "UTF-8")}&seller=${URLEncoder.encode(seller.removePrefix("@"), "UTF-8")}"
    ...
    suspend fun createMarketplaceListing(
        ...
        val mediaIds = mediaUris.distinct().take(12).map { sourceUri ->
            val mime = mediaMimeType(context, sourceUri)
            require(mime.startsWith("image/") || mime.startsWith("video/")) {
                "Marketplace media must be an image or video."
            }
            FynxProductionMessaging.uploadMedia(context, sourceUri, mime).getOrThrow().id
        }
        ...
        val raw = FynxBackendClient.postJson(
            ...
            JSONObject().apply {
                ...
                put("deliveryFee", deliveryFee ?: JSONObject.NULL)
    ...
    suspend fun createMarketplaceOrder(
        context: Context,
        listingId: String,
        quantity: Int
    ): Result<MarketplaceOrder> {
        val numericId = listingId.toLongOrNull()
        ...
        return FynxBackendClient.postJson(
            ...
            JSONObject().apply {
                put("listingId", numericId)
                put("quantity", quantity)
                put("orderId", java.util.UUID.randomUUID().toString())
            }.toString()
        ).mapCatching { parseOrder(JSONObject(it).getJSONObject("order")) }
    }
    ...
    private fun parseOrder(o: JSONObject): MarketplaceOrder {
        ...
        return MarketplaceOrder(
            ...
            o.optInt("quantity"),
            o.optDouble("unitPrice"),
            o.optDouble("deliveryFee"),
            o.optDouble("totalAmount"),
            o.optString("currency", "NGN"),
            product?.optString("title").orEmpty().ifBlank { o.optString("productTitle") },
            o.optString("sellerUsername").takeIf { it.isNotBlank() }
                ?: product?.optString("sellerUsername")?.takeIf { it.isNotBlank() },
            o.optString("status"),
            o.optString("trackingReference").takeIf { it.isNotBlank() },
            o.optString("fulfillmentMethod", "DELIVERY"),
            o.optJSONObject("shippingAddress"),
            o.optString("buyerNote"),
            o.optString("inspectionDeadline").takeIf { it.isNotBlank() },
            product?.optBoolean("deliveryAvailable", false) ?: false,
            product?.optBoolean("pickupAvailable", false) ?: false
        )
    }
    ...
    suspend fun setMarketplaceFulfillment(
        ...
        val safe = method.trim().uppercase()
        require(safe == "DELIVERY" || safe == "PICKUP") { "Choose delivery or pickup." }
        if (safe == "DELIVERY") {
            require(
        ...
        val shipping = if (safe == "DELIVERY") {
            JSONObject().apply {
                put("name", name.trim().take(120))
                put("phone", phone.trim().take(40))
                put("address", address.trim().take(500))
                put("city", city.trim().take(100))
                put("state", state.trim().take(100))
                put("country", country.trim().take(100))
            }
        } else {
            JSONObject.NULL
        }
        return FynxBackendClient.postJson(
            ...
            JSONObject().apply {
                put("method", safe)
                put("shippingAddress", shipping)
                put("buyerNote", buyerNote.trim().take(1000))
            }.toString()
        ).map { Unit }
    }

    suspend fun shipMarketplaceOrder(
        context: Context,
        id: String,
    ...
    suspend fun like(context: Context, id: String): Result<Pair<Boolean, Int>> {
        ...
        return FynxBackendClient.postJson(context, "/api/social/posts/$numericId/like", "{}").mapCatching {
            val o = JSONObject(it)
        ...
        return buildList {
            for (i in 0 until array.length()) {
                ...
                add(
                    MarketplaceListing(
                        o.optString("id"),
                        o.optString("seller_id"),
                        o.optString("seller_username"),
                        o.optString("seller_display_name"),
                        ...
                        o.optString("condition", "NEW"),
                        o.optInt("quantity"),
                        o.optString("location"),
                        o.optBoolean("delivery_available"),
                        o.optBoolean("pickup_available", true),
                        if (o.isNull("delivery_fee")) null else o.optDouble("delivery_fee"),
                        ids,
