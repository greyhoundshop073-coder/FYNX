...
privacy_backend = read("backend/privacyRoutes.js")
...
check("default messaging privacy enforces mutual follow", "messages_visibility" in privacy_backend and "social_follows WHERE follower_id=$1 AND followed_id=$2" in privacy_backend and "you must both follow each other to send a message" in privacy_backend)
...
check("Status music stays catalogue-only and persists real metadata", all(x in status_composer for x in ["selectedMusic?.id", "selectedMusic?.title", "selectedMusic?.artist", "selectedMusic?.durationMs"]))
check("Posts reject local music uploads and publish catalogue music metadata", 'require(music == null) { "Local music uploads are disabled.' in post_client and all(x in post_client for x in ["musicMediaId", "musicTitle", "musicArtist", "musicDurationMs"]))
