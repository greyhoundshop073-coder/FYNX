...
checks = [
    ...
    ("Composer can publish a valid music-only post", 'selectedCatalogueMusic != null' in composer and '"musicMediaId"' in client and 'catalogueMusic?.mediaId' in client),
    ...
    ("Successful publish clears the composer state", 'finishComposerAfterSuccess()' in composer and 'capturedUris = emptyList()' in composer and 'selectedCatalogueMusic = null' in composer),
    ...
    ("Music is controlled by the published catalogue", 'fynx_music_catalogue' in routes and 'musicMediaId' in bootstrap and 'published' in bootstrap),
    ("Music catalogue media is served through the controlled endpoint", '/api/social/music/media/:id' in routes and '/api/social/music/catalogue/' in composer),
