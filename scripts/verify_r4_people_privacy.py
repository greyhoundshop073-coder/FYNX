...
check("follow relationship is unique and self-follow is blocked", "PRIMARY KEY(follower_id,followed_id)" in profile and "cannot follow yourself" in journey or "cannot follow yourself" in social_hardening)
check("follow is blocked across mutual blocks", "FROM blocks WHERE (blocker_id=$1 AND blocked_id=$2) OR (blocker_id=$2 AND blocked_id=$1)" in journey or "FROM blocks WHERE (blocker_id=$1 AND blocked_id=$2) OR (blocker_id=$2 AND blocked_id=$1)" in profile)
check("follow/unfollow operations are backend-backed", "INSERT INTO social_follows" in journey and "DELETE FROM social_follows" in journey)
