...
check("follow relationship is unique and self-follow is blocked", "PRIMARY KEY(follower_id, followed_id)" in social and "cannot follow yourself" in social)
check("follow is blocked across mutual blocks", "SELECT 1 FROM blocks WHERE (blocker_id = $1 AND blocked_id = $2) OR (blocker_id = $2 AND blocked_id = $1)" in social and "follow unavailable" in social)
check("follow/unfollow operations are backend-backed", "INSERT INTO social_follows" in social and "DELETE FROM social_follows" in social)
