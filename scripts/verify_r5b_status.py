...
require('status interaction user ids match users','viewer_id BIGINT NOT NULL REFERENCES users(id)' in interactions and 'user_id BIGINT NOT NULL REFERENCES users(id)' in interactions and 'sender_id BIGINT NOT NULL REFERENCES users(id)' in interactions)
...
require('add status Recents filter','recentFilter' in hub and 'Photos' in hub and 'Videos' in hub and 'recentFilterMenu' in hub)
...
require('status viewer keyboard safe composer','navigationBarsPadding()' in timeline and 'imePadding()' in timeline and 'RoundedCornerShape(26.dp)' in timeline)
