...
check("Android AI preserves persisted conversation history without injecting a second welcome", 'messages = conversation.messages.map { AiMessage(it.text, it.role == "user") }.ifEmpty { listOf(welcome) }' in android_panel)
...
check("AI conversation route persists the agent reply", "const reply = agentResult.reply" in ai_routes and "text:reply" in ai_routes)
