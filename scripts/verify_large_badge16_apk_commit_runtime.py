...
runtime_required="[apk-only]" not in message
...
if runtime_required and not runtime.is_file(): fail.append("runtime certification README is missing")
apk_sha=""
apk_bytes=b""
if apk.is_file():
...
if runtime_required and sha and f"Commit: {sha}" not in runtime_text: fail.append("runtime evidence does not identify the exact commit SHA")
if runtime_required and run and f"Run: {run}" not in runtime_text: fail.append("runtime evidence does not identify the exact workflow run")
...
else: cert+=["","## Certified chain","- source commit is identified by GitHub Actions","- debug APK exists and contains a manifest plus DEX payload","- APK SHA-256 is recorded for exact download verification","- runtime evidence is required when the workflow requests full runtime certification","- no secrets or fabricated application records are introduced"]
